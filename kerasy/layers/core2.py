...
class Layers():
    ...
    def build(self, input_shape):
        ...
        self.w = np.c_[
            self.kernel_initializer(shape=(*self.output_shape, *self.input_shape)),
            self.bias_initializer(shape=(*self.output_shape,1))
        ]

class Input(Layers):
    def __init__(self, inputdim):
        super().__init__("Input", inputdim)

class Dense(Layers):
    def __init__(self, units, activation='linear', kernel_initializer='random_normal', bias_initializer='zeros'):
        """
        @param units             : (int) dimensionality of the output space.
